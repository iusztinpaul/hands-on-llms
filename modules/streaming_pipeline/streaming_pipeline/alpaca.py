...
class AlpacaSource(StatelessSource):
    def __init__(
        ...
        self._alpaca_client = AlpacaNewsStream(api_key=api_key, api_secret=api_secret, tickers=tickers)
...
class AlpacaNewsInput(DynamicInput):
    ...
    def __init__(self, tickers):
        ...
        try:
            self._alpaca_api_key = os.environ["ALPACA_API_KEY"]
        except KeyError:
            raise KeyError("ALPACA_API_KEY must be set as environment variable.")

        try:
            self._alpaca_api_secret = os.environ["ALPACA_API_SECRET"]
        except KeyError:
            raise KeyError("ALPACA_API_SECRET must be set as environment variable.")
